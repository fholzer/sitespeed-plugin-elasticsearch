...
class InfluxDBDataGenerator {
  ...
  dataFromMessage(message, time, alias) {
    function getTagsFromMessage(
      ...
      if (message.type.match(/(^pagexray|^coach|^browsertime|^thirdparty)/)) {
      ...
      if (message.url) {
        ...
        tags.group = util.toSafeKey(
          message.group,
          options.elasticsearch.groupSeparator || "_"
        );
      }
      return tags;
    }
    ...
    function getFieldAndSeriesName(key) {
      ...
      if (functions.indexOf(end) > -1) {
        return { field: end, measurement: keyArray.pop() };
      }
      return { field: 'value', measurement: end };
    }
    ...
    return reduce(
      ...
      (entries, value, key) => {
        ...
        entries.push({
          tags,
          measurement: fieldAndSeriesName.measurement,
          ...point
        });
        return entries;
      },
